...
**필터 칩 네 개가 아무것도 하지 않았다.** `중요만`·`음영 제외`·`전체`·`음영만` — 판정
함수 `keep()` 이 정의만 되어 있고 **호출하는 곳이 없어서**, 누르면 자기 눌림 표시만 바뀌고
어느 뷰도 달라지지 않았다. 폭을 가장 많이 먹으면서(네 개 414px) 화면이 흔들리는 원인이기도
했다 — 공항 뷰에서만 감췄으니 그 뷰만 바 크기가 달랐다. **지웠다**: 칩, `filter`, `keep()`,
`.noseg` 까지. 되살릴 거라면 **호출부부터** 만들어야 한다.

실측: 탭 줄이 단추 다섯 개 한 줄이 되고, 네 뷰 모두 바 높이 56px 와 폭 다섯 개가
완전히 같다. 지우기 전에는 768px 에서 106px, 375px 에서 156px 였다.
