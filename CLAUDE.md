...
그냥 무시되는 일이 있고, 그러면 버튼이 죽은 것처럼 보인다. 버튼 자신이 `정말 초기화?` 로
...
**줄을 넘긴다**(`flex-wrap:wrap`), 그리고 `.seg,.view,.rst { flex:0 0 auto; white-space:nowrap }`.

**공항 뷰에서 필터 칩을 `display:none` 으로 감추고 있었다.** 버튼이 사라지면 남은 것들이
폭을 다시 나눠 가져 **탭 줄 크기가 뷰마다 달라지고, 탭을 누를 때마다 화면이 흔들린다.**
자리는 그대로 두고 `opacity:.3; pointer-events:none` 으로 듣지 않는다는 것만 보인다.
실측: 네 뷰 모두 바 높이와 버튼 아홉 개 폭이 완전히 같다(768px 106px / 375px 156px).
