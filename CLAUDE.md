...
좌표가 명시된 고시를 전체 항로 위에 그린다. **타일 지도가 아니라 인라인 SVG**다 —
기내 와이파이가 끊기면 타일은 안 내려오는데 좌표는 이미 문서에 있다. 요약 보고서
상단의 `🗺 항로 개요도` 버튼을 눌렀을 때만 그린다.

**경도를 [-180,180]으로 접으면 안 된다.** 이 노선들은 날짜변경선을 넘으므로 그냥
찍으면 태평양 절반이 반대편 끝으로 간다. `unwrapTrack()`이 항적을 따라가며
이웃 간 차이를 [-180,180]으로 정규화해 **누적**한다(0 → −200처럼 연속). 낱개 좌표는
`placeLon()`이 가장 가까운 항적점을 찾아 그 점 기준으로 얹는다.

**개요도지 항법 자료가 아니다.** 등거리 원통도법이라 고위도가 늘어난다. 화면에
그렇게 적어 두었고, 거리 판정은 본문의 계산값을 봐야 한다.

**클래스 이름 충돌 주의.** 범례 칩에 `.mk`를 썼다가 원문 PDF 오버레이의
`.mk{position:absolute}`를 물려받아 범례 컨테이너가 높이 0으로 접혔다. 자식은
19px인데 부모가 0px이면 이걸 의심할 것. 범례는 `.lgi`로 분리했다.
